...
# global switch, use fixed max values for dim-less airfoil data_generation?
fixedAirfoilNormalization = True
# global switch, make data_generation dimensionless?
makeDimLess = True
...
######################################## DATA LOADER #########################################
#         also normalizes data_generation with max , and optionally makes it dimensionless              #

def LoaderNormalizer(data, isTest = False, shuffle = 0, dataProp = None):
    """
    # data_generation: pass TurbDataset object with initialized dataDir / dataDirTest paths
    # train: when off, process as test data_generation (first load regular for normalization if needed, then replace by test data_generation)
    # dataProp: proportions for loading & mixing 3 different data_generation directories "reg", "shear", "sup"
    ...
    if dataProp is None:
        ...
        if isTest:
            print("Reducing data_generation to load for tests")
        ...
        print("Number of data_generation loaded:", len(data.inputs) )
        ...
        for i in range(data.totalLength):
            if i >= (1-dataProp[3])*dataProp[0]:
                ...
                temp_1 = i + 1
                temp_2 = i + 1
        print("Number of data_generation loaded (reg, sup, shear):", temp_1, temp_2 - temp_1, i+1 - temp_2)
    ...
    # make dimensionless based on current data_generation set
    if makeDimLess:
        for i in range(data.totalLength):
...
class TurbDataset(Dataset):
    ...
    def __init__(self, dataProp=None, mode=TRAIN, dataDir="../data_generation/train/", dataDirTest="../data_generation/test/", shuffle=0, normMode=0):
        global makeDimLess, removePOffset
        """
        :param dataProp: for split&mix from multiple dirs, see LoaderNormalizer; None means off
        :param mode: TRAIN|TEST , toggle regular 80/20 split for training & validation data_generation, or load test data_generation
        :param dataDir: directory containing training data_generation
        :param dataDirTest: second directory containing test data_generation , needs training dir for normalization
        ...
        # load & normalize data_generation
...
# simplified validation data_generation set (main one is TurbDataset above)
