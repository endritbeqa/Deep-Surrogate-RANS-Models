...
class Base_Dataset(Dataset):
    def __init__(self, config: config_dict, mode: str):
        ...
        self.fixedAirfoilNormalization = (
            config.data_preprocessing.fixedAirfoilNormalization
        )
    ...
    def preprocess_data(self, data) -> np.ndarray:
        ...
        if not any(
            (self.removePOffset, self.makeDimLess, self.fixedAirfoilNormalization)
        ):
            return data
        ...
        v_norm = (
            np.max(np.abs(fields[0])) ** 2 + np.max(np.abs(fields[1])) ** 2
        ) ** 0.5
        ...
        if self.fixedAirfoilNormalization:
            ...
            max_targets_0 = (
                np.max(fields[3]) if np.max(fields[3]) != 0 else self.epsilon
            )
            max_targets_1 = (
                np.max(fields[4]) if np.max(fields[4]) != 0 else self.epsilon
            )
            max_targets_2 = (
                np.max(fields[5]) if np.max(fields[5]) != 0 else self.epsilon
            )
...
class Comparison_Dataset(Dataset):
    ...
    def __init__(self, config, mode):
        ...
        self.labels = np.array(
            [
                [[np.float32(re), np.float32(angle)] for angle in self.angles]
