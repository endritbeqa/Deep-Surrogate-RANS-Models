...
kernel_dx = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]) * (1 / 8)
kernel_dy = torch.Tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]]) * (1 / 8)
...
def get_force_vis(
    ux_field, uy_field, dx_shape, dy_shape, viscosity, cell_length=2 / 128
):
    ux_field = ux_field.T
    uy_field = uy_field.T
    ux_field = ux_field.view(1, 1, ux_field.shape[0], ux_field.shape[1])
    uy_field = uy_field.view(1, 1, uy_field.shape[0], uy_field.shape[1])
    vorticity = torch.nn.functional.conv2d(
        uy_field, kernel_dx, padding=1
    ) - torch.nn.functional.conv2d(ux_field, kernel_dy, padding=1)
    drag_x = torch.sum(dy_shape * vorticity * viscosity * cell_length)
    drag_y = torch.sum(-1 * dx_shape * viscosity * cell_length)
    return drag_x.item(), drag_y.item()


def get_lift_drag_coef(
    airfoil_shape, fields, AoA, velocity, viscosity=1e-5, density=1, cell_length=2 / 128
):
    pressure_field, ux_field, uy_field = fields
    with torch.no_grad():
        dx_shape, dy_shape = get_airfoil_unit_vector(airfoil_shape)
        force_pressure_x, force_pressure_y = get_force_pressure(
            pressure_field=pressure_field,
            dx_shape=dx_shape,
            dy_shape=dy_shape,
            cell_length=cell_length,
        )
        force_vis_x, force_vis_y = get_force_vis(
            ux_field=ux_field,
            uy_field=uy_field,
            dx_shape=dx_shape,
            dy_shape=dy_shape,
...
def reverse_preprocess_data(
    batch, removePOffset, makeDimLess, fixedAirfoilNormalization
) -> np.array:
