...
class GMM_VAEBottleneck(nn.Module):
    ...
    def log_normal_diag(self, x, mu, log_var, dim=None):
        D = x.shape[1]
        log_p = -0.5 * D * torch.log(torch.tensor(2) * torch.pi) - 0.5 * log_var - 0.5 * torch.exp(-log_var) * (x - mu) ** 2.
    ...
    def gmm_loss(self, z, mean, logvar):
        ...
        prior_log_p = self.log_normal_diag(z, prior_means, prior_logvars) + torch.log(w)  # K x B x L
        prior_log_prob = torch.logsumexp(prior_log_p, dim=0, keepdim=False)  # B x L

        return torch.mean((torch.exp(prior_log_prob)*(prior_log_prob - encoder_log_prob)).mean(-1))
    ...
    def sample(self, num_samples):
        ...
        for i in range(num_samples):
            ...
            if i == 0:
                ...
                z = torch.cat((z, self.means[[indx]] + eps[[i]] * torch.exp(self.logvars[[indx]])), 0)
        return z
