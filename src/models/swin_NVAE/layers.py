...
from src.models.modeling_swinV2 import Swinv2Layer
from src.models.modeling_swinV2 import Swinv2PatchMerging
...
class Swin_Decoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i_layer in range(self.num_layers):
            ...
            layers.append(stage)
        self.last_layer = Conv_layer(config.embed_dim*2, config.output_dims, (config.image_size, config.image_size))
        layers.append(self.last_layer)
        self.layers = nn.ModuleList(layers)

    def forward(self, hidden_states: torch.Tensor, skip_connections, input_dimensions: Tuple[int, int]):
