...
from src.models.uncertainty.diffusion.Swin_UNet.layers import Conv_layer, Swin_Decoder
...
class Decoder(nn.Module):
    ...
    def forward(self, x, skip_connections, time):
        B, L, C = x.shape
        H = W = int(math.sqrt(L))
        x = self.decoder(x, skip_connections, time, (H, W))
        if self.config.input_res_skip:
            x = self.second_last_layer(x, time)
            ...
            x = self.last_layer(x, time, reshape=False)
        else:
            x = self.last_layer(x, time, reshape=True)
        return x