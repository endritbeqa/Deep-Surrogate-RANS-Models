...
def get_config():
    ...
    config.encoder.num_channels = (
        config.encoder.conv_output_dim
        if config.encoder.input_res_skip
        else config.encoder.conv_input_dim
    )
    ...
    config.middle_block.dim = config.encoder.embed_dim * (
        2 ** (len(config.encoder.depths))
    )
    config.middle_block.depth = 2
    config.middle_block.num_heads = 16
    config.middle_block.input_res = (
        config.image_size
        // (config.encoder.patch_size * 2 ** (len(config.encoder.depths))),
        config.image_size
        // (config.encoder.patch_size * 2 ** (len(config.encoder.depths))),
    )
