...
def plot_moment_comparison(
    targets, predictions, file_name, output_dir, plot_delta=True
):
    ...
    for col in range(C):
        ...
        for row in range(rows):
            im = axes[row, col].imshow(
                data[row, col], cmap=cm.magma, vmin=vmin, vmax=vmax
            )
...
def save_parameter_comparison(predictions, parameters, output_dir):
    ...
    for i in range(num_Angles):
        ...
        save_path = os.path.join(
            reynolds_comparison_folder, "Reynolds_comparison_at_{}.png".format(angle)
        )
        plt.savefig(save_path)
        plt.close()
    ...
    for i in range(num_REs):
        ...
        save_path = os.path.join(
            angle_comparison_folder, "Angle_comparison_at_{}.png".format(re)
        )
        plt.savefig(save_path)
        plt.close()
    ...
    for i, (mode_region, mse_lists) in enumerate(data.items()):
        ...
        for i, (label, mse_list) in enumerate(mse_lists):
            ...
            axes[row, col].fill_betweenx(
                ratios, min_curve, max_curve, color=colors[i], alpha=0.1
            )
...
def plot_drag_coefficient_distribution(
    label, targets, predictions, output_dir, num_buckets
):
    plt.figure(figsize=(10, 6))
    ...
    intervals = [
        (min_val + x * step, min_val + (x + 1) * step) for x in range(num_buckets)
    ]
    target_counts = np.array(
        [np.sum((targets >= low) & (targets < high)) for low, high in intervals]
    )
    target_counts = target_counts / np.sum(target_counts)
    prediction_counts = [
        [np.sum((run > low) & (run <= high)) for low, high in intervals]
        for run in predictions
    ]
    prediction_counts = np.array(prediction_counts)
    prediction_counts = prediction_counts / np.sum(
        prediction_counts, axis=1, keepdims=True
    )
    ...
    for i, x in enumerate(x_vals):
        plt.plot(
            [x, x],
            [prediction_counts_mins[i], prediction_counts_maxs[i]],
            color=colors[0],
            linestyle="--",
            label="Prediction",
        )
        plt.plot(
            [x - step / 100, x + step / 100],
            [prediction_counts_mins[i], prediction_counts_mins[i]],
            color=colors[0],
            linestyle="solid",
        )
        plt.plot(
            [x - step / 100, x + step / 100],
            [prediction_counts_maxs[i], prediction_counts_maxs[i]],
            color=colors[0],
            linestyle="solid",
        )
...
def plot_samples_different_models(
    samples, chart_label, row_labels, channel, output_dir
):
    samples = to_numpy(samples)
    ...
    for col in range(B):
        vmin, vmax = (
            samples[:, col, channel, :, :].min(),
            samples[:, col, channel, :, :].max(),
        )
        axes[0, col].set_title(column_labels[col], fontsize=12, pad=8)
        for row in range(Models):
            ax = axes[row, col]
            im = ax.imshow(
                samples[row, col, channel], cmap=cm.magma, vmin=vmin, vmax=vmax
            )
            ax.set_xticks([])
            ax.set_yticks([])
        cbar = fig.colorbar(
            im, ax=axes[:, col], orientation="horizontal", pad=0.03, shrink=0.8
        )

    fig.suptitle("{} samples of models and ground truth".format(channel_name[channel]))
    plt.savefig(
        os.path.join(output_dir, channel_name[channel], "{}.png".format(chart_label))
    )
...
def plot_moment_comparison_models(moments, file_name, output_dir):
    ...
    for col in range(C):
        ...
        for row in range(rows):
            im = axes[row, col].imshow(
                moments[row, col], cmap=cm.magma, vmin=vmin, vmax=vmax
            )
...
def plot_bar_chart(
    statistics, categories, bar_labels, chart_label, x_label, output_dir
):
    ...
    for i in range(num_groups):
        axes[0].bar(
            x + i * width - width,
            interpolation_means[:, i],
            width,
            yerr=interpolation_stds[:, i],
            capsize=5,
            color=colors[i],
            label=bar_labels[i],
            alpha=0.7,
    ...
    for i in range(num_groups):
        axes[1].bar(
            x + i * width - width,
            extrapolation_means[:, i],
            width,
            yerr=extrapolation_stds[:, i],
            capsize=5,
            color=colors[i],
            label=bar_labels[i],
            alpha=0.7,
...
def plot_sampling_speed_bar_chart(
    statistics, categories, num_samples_compared, bar_labels, chart_label, output_dir
):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i in range(num_groups):
        axes[0].bar(
            x + i * width - width,
            num_samples_1_means[:, i],
            width,
            yerr=num_samples_1_stds[:, i],
            capsize=5,
            color=colors[i],
            label=bar_labels[i],
            alpha=0.7,
    ...
    for i in range(num_groups):
        axes[1].bar(
            x + i * width - width,
            num_samples_2_means[:, i],
            width,
            yerr=num_samples_2_stds[:, i],
            capsize=5,
            color=colors[i],
            label=bar_labels[i],
            alpha=0.7,
    ...
    for row, (model, ratio_statistics) in enumerate(data.items()):
        ...
        for col, (mode_region, mse_lists) in enumerate(ratio_statistics.items()):
            ...
            for j, (label, mse_list) in enumerate(mse_lists):
                ...
                axes[row, col].fill_betweenx(
                    ratios, min_curve, max_curve, color=colors[j], alpha=0.1
                )

            if row == 0:
                axes[row, col].set_title(
                    "{} Uncertainty MSE".format(mode_region), fontsize=12
                )
            if col == 0:
